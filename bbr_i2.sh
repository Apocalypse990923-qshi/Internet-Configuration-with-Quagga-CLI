...
START_TIME=$(($(date +%s)))
echo "START SEND TIME: $START_TIME"
FILE_SIZE=$(($(stat --printf="%s" $SRC_FILE1)))
echo "Src File size: $FILE_SIZE Bytes"
sleep 60
...
END_TIME1=($(stat -c %Y $DST_FILE1))
END_TIME2=($(stat -c %Y $DST_FILE2))
echo "SEND END TIME1: $END_TIME1"
# echo "SEND END TIME2: $END_TIME2"
SEND_TIME1=$(($(stat -c %Y $DST_FILE1)-START_TIME)) 
SEND_TIME2=$(($(stat -c %Y $DST_FILE2)-START_TIME)) 

DST1_FILE_SIZE=$(($(stat --printf="%s" $DST_FILE1)))
DST2_FILE_SIZE=$(($(stat --printf="%s" $DST_FILE2)))
# echo "Dst1 File size: $DST1_FILE_SIZE Bytes"
# echo "Dst2 File size: $DST2_FILE_SIZE Bytes"
# echo "SEND_TIME1: $SEND_TIME1 sec"
# echo "SEND_TIME2: $SEND_TIME2 sec"
# echo "I2 Throughput: $(($FILE_SIZE * 16 / ($SEND_TIME1 + $SEND_TIME2))) bps"
echo "Dst File size: $DST1_FILE_SIZE Bytes"
echo "SEND_TIME: $SEND_TIME1 sec"
echo "I2 Throughput: $(($FILE_SIZE * 8 / ($SEND_TIME1))) bps"
