...
def load_config_to_router(router_name, config_file_paths):
    ...
    print("Parsing files: {0} and {1}".format(config_file_paths[0],config_file_paths[1]))
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Usage: python load_configs.py <config_directory_path>")
        sys.exit(1)

    config_base_path = sys.argv[1]
    routers = ["ATLA","CHIC","HOUS","KANS","LOSA","NEWY","SALT","SEAT","WASH"]
    for router in routers:
        config_files = [os.path.join(config_base_path, router, "zebra.conf.sav"), os.path.join(config_base_path, router, "ospfd.conf.sav")]
